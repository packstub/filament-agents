...
| **Live tables and charts** | `show-table` renders the resource's own Filament table under the answer, with its sorting and row actions (search, filters and pagination once a result runs past ten rows). `draw-chart` and any tool result with a `chart` key render a chart. The "Ask …" button carries the record being viewed into the chat as page context. |
